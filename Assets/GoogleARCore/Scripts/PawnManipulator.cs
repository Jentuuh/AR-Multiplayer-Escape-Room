...
namespace GoogleARCore.Examples.ObjectManipulation
{
    ...
    public class PawnManipulator : Manipulator
    {
        ...
        public Button changePrefabButton;
        ...
        private void Start() {
            updateButtonText(prefabsList[placeID].name);
        }
        ...
        public void togglePlacer() {
            if (placeID + 1 < prefabsList.Count)
                placeID++;
            else
                placeID = 0;

            // Check if current element is empty
            if (prefabsList[placeID] == null)
                updateButtonText("None");

           updateButtonText(prefabsList[placeID].name);
        }

        private void updateButtonText(string text) {
             changePrefabButton.transform.Find("Text").GetComponent<Text>().text = text;
        }
